 ...
 * /imports/epub
 *
 * Guided EPUB import: parse cookbook files locally, let the user confirm likely
 * recipe candidates, then save only selected recipes.
 */

import { useMemo, useRef, useState } from "react";
import { Link, useNavigate } from "react-router";
import type { Route } from "./+types/imports.epub";
import { requireUser } from "~/lib/auth.server";
import { parseEpubArchive, type EpubRecipeCandidate } from "~/lib/epub-parser";
...
export function meta() {
  return [{ title: "Import EPUB cookbook — ProjectSpice" }];
}
...
export default function ImportEpub() {
  const navigate = useNavigate();
  const inputRef = useRef<HTMLInputElement>(null);
  const [step, setStep] = useState<Step>("idle");
  const [candidates, setCandidates] = useState<EpubRecipeCandidate[]>([]);
  ...
  async function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    setStep("parsing");
    setError(null);
    setCookbookName(file.name.replace(/\.epub$/i, "").replace(/[-_]+/g, " "));

    try {
      const buffer = await file.arrayBuffer();
      const parsed = parseEpubArchive(new Uint8Array(buffer));
      if (parsed.length === 0) {
        setStep("error");
        setError("No likely recipes found. This EPUB may need the PDF/OCR importer or manual entry.");
        return;
      }
  ...
  function updateCandidate(id: string, patch: Partial<EpubRecipeCandidate>) {
  ...
  async function handleImport() {
    ...
    const response = await fetch("/api/imports/epub", {
      ...
      body: JSON.stringify({ cookbookName: cookbookName.trim(), recipes: selected }),
    });
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-10 bg-background/95 backdrop-blur border-b">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center gap-3">
          ...
          <span className="font-medium text-sm">Import EPUB cookbook</span>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 space-y-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold">Import EPUB Cookbook</h1>
            <p className="text-sm text-muted-foreground mt-1">
              Review detected recipe sections before they become recipes.
            </p>
          </div>
        ...
        {step === "parsing" && (
          <div className="flex items-center gap-3 py-8">
            <Spinner />
            <span className="text-sm text-muted-foreground">Scanning EPUB spine and chapters…</span>
          </div>
        )}
        ...
        {step === "importing" && (
          <div className="flex items-center gap-3 py-8">
            <Spinner />
            <span className="text-sm text-muted-foreground">Saving confirmed recipes…</span>
          </div>
        )}
        ...
        {step === "done" && result && (
          <div className="rounded-lg border bg-card px-5 py-4 space-y-4 max-w-2xl">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {result.imported} EPUB recipes imported.
              </p>
            </div>
            {result.errors.length > 0 && (
              <details className="text-sm">
                <summary className="cursor-pointer text-amber-700 font-medium">
                  {result.errors.length} recipes had errors
                </summary>
...
function RecipeEditor({
  ...
  candidate: EpubRecipeCandidate;
  onChange: (patch: Partial<EpubRecipeCandidate>) => void;
}) {
  return (
